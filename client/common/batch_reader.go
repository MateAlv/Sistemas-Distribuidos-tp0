...
func NewBatchReader(filePath string, agency string, batchSize int) (*BatchReader, error) {
...
func (br *BatchReader) ReadNextBatch() ([]*Bet, error) {
    ...
    for len(batch) < br.batchSize && br.scanner.Scan() {
        br.lineNumber++
        line := strings.TrimSpace(br.scanner.Text())
        
        bet, err := br.parseCSVLine(line)
        if err != nil {
            log.Warnf("Skipping line %d: %v", br.lineNumber, err)
            continue
        }
...
func (br *BatchReader) parseCSVLine(line string) (*Bet, error) {
    ...
    if len(records) != 5 {  // name,lastname,document,birthdate,number
...
func SerializeBatch(bets []*Bet) string {
    ...
    return strings.Join(serializedBets, "\n")
}